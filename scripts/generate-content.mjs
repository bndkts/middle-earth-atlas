...
import { drawMoria } from './draw-moria.mjs';
...
export function buildOutputs() {
  validatePublication();
  const out = new Map();
  out.set('assets/moria-section.svg', drawMoria());
  ...
  for (const p of places) {
    ...
    putPage(`/places/${p.id}/`,p.n,`${p.n} in ${p.r}: location, history, alternative names and sources, with a direct link to the interactive Middle-earth map.`,
      `<p class="eyebrow">${e(p.r)} · ${e(p.t)}</p><p class="lead">${e(p.d)}</p><p>${link('/?place='+p.id,'Show '+p.n+' on the map')}</p>${p.id === 'moria' ? '<p>'+link('/maps/moria/','Explore the illustrated halls of Moria')+'</p>' : ''}
  ...
  const urls = ['/','/maps/moria/',...Array.from(out.keys()).filter(f=>f.endsWith('/index.html')).map(f=>'/'+f.slice(0,-10))];
